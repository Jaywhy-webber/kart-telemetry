...
The output is a single self-contained HTML file (e.g. `driver_a_dashboard.html` or `driver_a_vs_driver_b_dashboard.html`) written to the same directory as the CSVs. Satellite tile images are embedded as base64 JPEG, so the file works fully offline after generation. An internet connection is only required during the build step to download tiles.
...
`errors="coerce"` turns every non-numeric cell into `NaN`. `dropna(subset=["Time"])` removes any row with no valid timestamp, which covers the units row and any trailing empty rows.
...
**Out-lap and in-lap trimming (beacon mode only):** The first and last segments are unconditionally removed, since they are the warm-up and cool-down laps.

**Outlier lap filter (both modes):** Any lap more than 3 seconds slower than the median is removed. This catches traffic, spins, and red flags:
...
`rotation_events()` pre-filters to `0 < GPS Radius < 500 m`, which avoids division errors on straights and spurious results from near-straight sections.
...
4. Computes 2D FFT cross-correlation between GPS density and satellite edges; the peak marks the optimal pixel shift
...
`_applyViewportOne(divId, dLat, dLon)` looks up the figure's own zoom from its align bar (`data-zoom`) and computes `dx, dy` in pixels independently per figure. It then calls `Plotly.relayout` to shift both the axis ranges and the layout image origin, so the GPS traces appear to move while the satellite stays fixed.
                                        